...
def handle_missing_and_outliers(x, outlier_coef):
    ...
    if outlier_coef != -1:
        for i in range(x.shape[1]):
            std = np.std(res_x[:, i])
            lower, upper = (
                mean_of_non_null[i] - outlier_coef * std,
                mean_of_non_null[i] + outlier_coef * std,
            )
...
def add_log(x, columns=[]):
    ...
    for c in columns:
        mask = np.where(res[:, c] != -999)
        data = res[mask, c].copy()
        data[data <= 0] = 1
        res[mask, c] = np.log(data)
    return res


def apply_preprocessing(
    tr_x, te_x, corr_tol=0.01, outlier_coef=2.0, degree=1, log_cols=[]
):
    """
